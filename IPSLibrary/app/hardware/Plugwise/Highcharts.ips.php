<?
	$version = "2.01"; $versionDate = "12.05.2012";
	...
	//	--- V2.01 ---------------------------------------------------------------------------------------------------------------------
	// 07.05.2012  KHC   NEU      Test mit Integration Highstocks. Neuer Parameter ['Ips']['ChartType'] = 'Highcharts' oder 'Highstock'
...
//			DebugString(my_json_encode($cfgArr));
	...
	function GetHighChartsLangOptions($cfg)
	{
		...
		if (isset($cfg['lang']))
			$cfgArr['lang'] = $cfg['lang'];

		// Default
		if (!isset($cfg['lang']['decimalPoint']))
			$cfgArr['lang']['decimalPoint'] = ",";
		if (!isset($cfg['lang']['thousandsSep']))
			$cfgArr['lang']['thousandsSep'] = ".";

		if (!isset($cfg['lang']['months']))
			$cfgArr['lang']['months'] = array('Januar', 'Februar', 'März', 'April', 'Mai', 'Juni', 'Juli', 'August', 'September', 'Oktober', 'November', 'Dezember');
		if (!isset($cfg['lang']['shortMonths ']))
			$cfgArr['lang']['shortMonths '] = array('Jan', 'Feb', 'Mär', 'Apr', 'Mai', 'Jun', 'Jul', 'Aug', 'Sep', 'Okt', 'Nov', 'Dez');
		if (!isset($cfg['lang']['weekdays']))
			$cfgArr['lang']['weekdays'] = array('Sonntag', 'Montag', 'Dienstag', 'Mittwoch', 'Donnerstag', 'Freitag', 'Samstag');

		$s = my_json_encode($cfgArr);
		$s = substr($s, 1, -1); // die beiden '{ }' am Ende entfernen
		return $s;
	}
